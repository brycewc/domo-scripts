 ...
 * @param {string} [options.idFlag]  - Explicit single-ID flag name, overriding "<name>-id" (e.g. "id" for --id)
 * @param {string} [options.idsFlag] - Explicit multi-ID flag name, overriding "<name>-ids" (e.g. "ids" for --ids)
...
function resolveIds(argv, options) {
	...
	const idFlag = options.idFlag || `${name}-id`;
	const idsFlag = options.idsFlag || `${name}-ids`;
